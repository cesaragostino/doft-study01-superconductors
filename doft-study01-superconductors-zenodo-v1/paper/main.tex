...
physiological control and nonequilibrium statistical mechanics~\cite{Mori1965,
Zwanzig2001,Efron1993,Tinkham1996,BCS1957}.  The \DOFT{} program borrows 
...
Figure~\ref{fig:residual_means} and~\ref{fig:residual_vs_d} summarizes 
the logarithmic residuals $\logres$ by family and sub-network. The mean 
residuals are close to zero in most groups, with standard deviations at the 
level expected from the calibration set.
...
Figure~\ref{fig:kappa_hist} shows the distribution of the difference in
post-correction error
$\Delta \mathrm{err} = \mathrm{err}_\kappa - \mathrm{err}_\eta$
across all cluster-core jumps, while
Figure~\ref{fig:kappa_top} highlights the ten jumps with the
largest $|\Delta \mathrm{err}|$.
...
The locking grammar used here did not arise from post-hoc pattern hunting
over superconducting data. It was first developed in a separate line of
work on delayed oscillator networks, where one asks how a hierarchy of
coupled resonant modes with memory can maintain coherence across widely
separated scales. In that setting, discrete ``locks'' between effective
oscillation periods appear naturally, and a convenient coarse-grained
description of those locks is a multiplicative basis built from small
primes \(\{2,3,5,7\}\) with constrained non-negative exponents.

In the present study we do not re-derive that structure from first
principles, nor do we claim that the primes themselves have any privileged
status in Nature. We treat the delayed-oscillator locks as an \emph{a
priori} hypothesis about how coherent scales might organize, and then pose
a purely empirical question: given this fixed grammar and a global
correction law, can a heterogeneous collection of superconductors and
superfluid helium be compressed into a small number of stable
prime-space fingerprints? At this level, the construction is best viewed
as a phenomenological compression scheme, not as a microscopic theory of
superconductivity.

The main outcome of this study is therefore not a new microscopic model,
but an empirical statement: a simple locking grammar on the primes
\(\{2,3,5,7\}\), combined with a universal two-parameter correction law
calibrated once on classical metals, is sufficient to organize a
heterogeneous set of superconductors and superfluid helium into stable
prime-space fingerprints. These fingerprints remain robust under changes
in the dataset (the v6 catalogue versus earlier runs), under variation of
technical parameters (window size \(w\) and prime cutoff), and under the
inclusion of a weak coupling \(\kappa\). The latter produces only small,
localized refinements concentrated in MgB\(_2\), leaving the global
fingerprint structure essentially unchanged.

From a \DOFT{} perspective, this suggests that if a delayed-oscillator
network underlies these systems, it may operate with a small, discrete
set of preferred ratios and a universal correction that is largely
insensitive to microscopic details. From a more conventional condensed
matter perspective, the fingerprints can be read simply as a compact,
phenomenological summary of how different families sit relative to the
same discrete lattice of scales.

Overall, we suggest that the delayed-oscillator grammar plus a universal
correction law is a useful way to organize a heterogeneous superconductivity
dataset, and we have documented several empirical regularities that would
be hard to obtain from an unconstrained fit. Whether these patterns point
to a deeper oscillator-based structure or simply reflect hidden
regularities in curated experimental data remains an open question. The
present work should therefore be read as a first delivery: a concrete
proposal plus a set of sanity checks that can be sharpened, falsified,
or extended in subsequent studies.
